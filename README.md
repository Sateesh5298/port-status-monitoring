# Port Status Monitoring Tool using SDN (POX)

## 📌 Project Overview

This project implements a Port Status Monitoring system using Software Defined Networking (SDN). It detects real-time link failures (port down) and recoveries (port up) in a network using a centralized controller.
...
* Detect link failure and recovery
* Analyze network behavior (packet loss)
* Maintain log of port status changes
...
* No alternate path (easy validation)
...
```bash
cd ~/pox
python3.8 pox.py forwarding.l2_learning port_monitor
```
...
* Link Down → 100% packet loss
* Controller logs events correctly
...
This project demonstrates real-time monitoring of network port status using SDN and validates controller-based network management.
